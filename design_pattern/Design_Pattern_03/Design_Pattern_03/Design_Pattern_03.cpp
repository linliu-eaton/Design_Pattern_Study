...
namespace
{
    ...
    std::condition_variable queue_not_full; // producer
    std::condition_variable queue_not_empty; // comsuer
    ...
    private:
        static constexpr size_t MAX_QUEUE_SZIE{ 100 };
        std::queue<int> m_que;
    };
    ...
    void producer(int id)
    {
        ...
        while (1)
        {
            ...
            queue_not_full.wait(lock, [] {
                                             bool ret = g_queue.full();
                                             if (ret) {
                                                 std::cout << "producer: queue NOT ready...\n";
                                             }
                                             else {
                                                 std::cout << "producer: queue ready...\n";
                                             }
                                             return !ret;
                                         }
            );
            ...
            std::this_thread::sleep_for(std::chrono::milliseconds(2000));  // construct queue full/empty
        }
    }
    ...
    void consumer(int id)
    {
        ...
        while (1)
        {
            ...
            queue_not_empty.wait(lock, [] { 
                                            bool ret = g_queue.empty(); 
                                            if (ret) {
                                                std::cout << "consumer: data NOT ready now...\n";
                                            }
                                            else
                                            {
                                                std::cout << "consumer: data ready now...\n";
                                            }
                                            return !ret;
                                          }
            );
